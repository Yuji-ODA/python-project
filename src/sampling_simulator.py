...
def main(population1, population2):
    ...
    for sampling_rate in np.arange(0.1, 1, 0.1):
        ...
        p12_actual, p1_actual, p2_actual = np.array((n12_actual, n1_actual, n2_actual)) / n_all_actual

        # 理論値の計算
        n12_expected, n1_expected, n2_expected = sampling_rate * np.array((n12, n1, n2))
        ...
        # サンプリングの総数から重複分を引く
        n1_computed, n2_computed = np.array((size1, size2)) * sampling_rate - n12_computed
        n_all_computed = n1_computed + n2_computed + n12_computed
        p12_computed, p1_computed, p2_computed = np.array((n12_computed, n1_computed, n2_computed)) / n_all_computed

        # 補正
        # 重複分の取りこぼしを補正
        n12_corrected = n12_actual / sampling_rate
        # 重複分を増やした分だけ減らす
        n_all_corrected, n1_corrected, n2_corrected = np.array((n_all_actual, n1_actual, n2_actual)) - (n12_corrected - n12_actual)
        p12_corrected, p1_corrected, p2_corrected = np.array((n12_corrected, n1_corrected, n2_corrected)) / n_all_corrected

        print_result('expected ', n12_expected, n1_expected, n2_expected, p12_expected, p1_expected, p2_expected)
        print_result('actual   ', n12_actual, n1_actual, n2_actual, p12_actual, p1_actual, p2_actual)
        print_result('computed ', n12_computed, n1_computed, n2_computed, p12_computed, p1_computed, p2_computed)
        print_result('corrected', n12_corrected, n1_corrected, n2_corrected, p12_corrected, p1_corrected, p2_corrected)


def print_result(header, n12, n1, n2, p12, p1, p2):
    print(f'{header}: n12 = {round(n12)}, n1 = {round(n1)}, n2 = {round(n2)}, '
          f'p12 = {p12:.4f}, p1 = {p1:.4f}, p2 = {p2:.4f}')
